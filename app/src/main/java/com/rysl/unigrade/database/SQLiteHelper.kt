...
class SQLiteHelper(context: Context): SQLiteOpenHelper(context, DATABASE_NAME, null, 1) {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        db?.execSQL("CREATE TABLE ${TABLE_NAMES[1]} (subID INTEGER NOT NULL, modID INTEGER NOT NULL)")
        db?.execSQL("CREATE TABLE ${TABLE_NAMES[2]} (ID INTEGER PRIMARY KEY NOT NULL, NAME TEXT NOT NULL)")
        db?.execSQL("CREATE TABLE ${TABLE_NAMES[3]} (modID INTEGER NOT NULL, asgID INTEGER NOT NULL)")
        db?.execSQL("CREATE TABLE ${TABLE_NAMES[4]} (ID INTEGER PRIMARY KEY NOT NULL, NAME TEXT NOT NULL, PERCENTAGE INTEGER NOT NULL, TYPE BOOLEAN NOT NULL, RESULT INTEGER, DESCRIPTION TEXT)")
        createTutorial()
    }
